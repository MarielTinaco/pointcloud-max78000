...
if __name__ == "__main__":
    ...
    for f in folders:
        ...
        test_i = indices[int(len(indices)*0.7):int(len(indices)*0.9)]   # TEST  indices
        inference_i = indices[int(len(indices)*0.9):]                   # INFERENCE  indices
        ...
        INF_DIR = os.getcwd()+'/assets/depthmap_data/DepthMap/inference/'+f+'/'
        if os.path.exists(INF_DIR) == False:
            print('Creating Folder:   ',INF_DIR)
            os.makedirs(INF_DIR)

        for index in inference_i:
            print(fname[index])
            raw_img = Image.open(filelist[index])

            left,center,right = cropper(img = raw_img)
            
            plt.imsave(INF_DIR+fname[index]+'_L.png', np.asarray(left), dpi=1, cmap='gray')
            plt.imsave(INF_DIR+fname[index]+'_C.png', np.asarray(center), dpi=1, cmap='gray')
            plt.imsave(INF_DIR+fname[index]+'_R.png', np.asarray(right), dpi=1, cmap='gray')
